...
把新題庫鏈的活動真的「玩起來」的最小前端（P0 垂直切片）：抓 companion-api 的
`GET /v1/units/:id/activity-set`，一題一題呈現、判分、把作答回報 companion-api。
...
# → http://localhost:4321/?unit=N-5-4&limit=10
```
...
test/             node --test（adapter / render / grade）
```
